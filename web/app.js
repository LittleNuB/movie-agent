...
const statuses={idle:'等待你的想法',running:'导演处理中',waiting_review:'等你审核',stopped:'制作已中止',pending:'待执行',submitting:'正在提交',queued:'云端排队中',downloading:'保存素材中',rendering:'合成中',succeeded:'已完成',failed:'受阻',unknown:'提交结果待核对',cancelled_local:'本地已停止'};
...
function renderArtifact(a){
  ...
  if(a.kind==='script'){
    ...
    return `<div class="script-pane"><div class="script-toolbar"><span class="save-state" id="save-state">${edit?.dirty?'有未保存修改':p.draft.submitted_revision===p.draft.revision?'已提交草稿':'保存与提交分开'}</span>${button('save-script','保存草稿')}${button('submit-script','提交修改')}</div><textarea id="script-editor" class="script-editor" aria-label="剧本编辑器">${esc(text)}</textarea><div class="artifact-footnote">${toolbar}保存不触发制作。<details><summary>查看此版本原文</summary><pre class="runtime-document">${esc(a.text)}</pre></details>${reviewBlock}</div></div>`;
  }
  if(['film','trial','video'].includes(a.kind))return `<div class="document">${toolbar}<h1>${esc(a.title)}</h1><video id="film-player" data-artifact="${a.id}" class="runtime-video" src="/api/media/${a.id}" controls preload="metadata"></video><div class="runtime-actions">${button('annotate',icon('note')+'标注当前时间点')} ${download} ${button('history','版本记录')}${button('adopt','从此版本继续',`data-id="${a.id}"`)}</div><div class="runtime-status">${a.meta.media?.duration?.toFixed(1)||'?'} 秒 · ${a.meta.picture_master_id?'保留独立画面母版':'生成源片段'}</div>${reviewBlock}${a.meta.edit?`<details><summary>采用素材与声音</summary><pre class="runtime-json">${esc(JSON.stringify(a.meta.edit,null,2))}</pre></details>`:''}</div>`;
...
async function saveScript(){
  ...
  const request=api(`/projects/${pid}/draft`,'PUT',{text,expected_revision:edit?.base??p.draft.revision});saving.set(pid,request);
...
document.addEventListener('input',e=>{
  if(e.target.id==='script-editor'){const prior=draftEdits.get(activeId);draftEdits.set(activeId,{...prior,text:e.target.value,base:prior?.base??snapshot.project.draft.revision,dirty:true});$('#save-state').textContent='有未保存修改';clearTimeout(autosaveTimer);autosaveTimer=setTimeout(()=>saveScript().catch(err=>notice(err.message)),800);}
});
...
document.addEventListener('click',async e=>{
  ...
  try{
    ...
    if(action==='submit-script'){const saved=await saveScript();await api(`/projects/${activeId}/draft/submit`,'POST',{expected_revision:saved.revision});notice('已提交剧本，导演将先理解并给出修改方案。');await refresh();return;}
