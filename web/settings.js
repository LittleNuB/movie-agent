...
function chrome(body){modal('模型与 API',`<nav class="settings-tabs"><button data-setting="connections" class="${page==='connections'?'active':''}">服务连接</button><button data-setting="assignments" class="${page==='assignments'?'active':''}">用途分配</button></nav>${body}<p class="runtime-config-note">生成规格：MiniMax 视频 768P，方舟视频 720P。成片导出 720P，已有素材继续复用。</p><p class="runtime-config-note">Key 仅保存在此 Windows 用户的凭据管理器中。保存不调用模型；连接检查不生成媒体。</p>`,true);}
