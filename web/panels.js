...
export function artifactPanel(a,snapshot,artifact){
 ...
 const head=`<div class="artifact-meta"><span>${esc(kinds[a.kind]||a.kind)} · ${date(a.created)}</span><span class="version-badge ${adopted?'adopted':''}">${adopted?'当前采用':r?'待审核版本':'保留版本'}</span></div><h1>${esc(a.title)}</h1>`;
