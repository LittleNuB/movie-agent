...
**更新：** 2026-09-06。产品与关键交互方向已确认；独立分支已实现可点击原型，用户体验审阅待完成。main 保留文档，尚无电影 Agent 运行时或真实影片能力。
...
当前交付为[可点击原型](./docs/plans/clickable-prototype.md)，位于独立分支；浏览器检查通过，仍需用户实际操作审阅。下一步根据反馈收敛交互，再衔接真实运行时任务。可舍弃原型不合并为正式运行时。

电影运行时接口、数据库 Schema、消息交付与任务恢复机制、具体代码复用、最终角色数量、独立音频 Provider、安装分发与硬件要求仍待相应技术任务确定。首轮验证矩阵的范围已确认，具体样本和执行未完成。赛事条件与日程在交付前重新核验。

本次建仓只建立文档与配置示例。过去提出的固定七 Agent、费用硬上限、必须保留旧 Pi 工程、外部剪辑工程强制导出均已撤回，不应从历史资料重新引入。
